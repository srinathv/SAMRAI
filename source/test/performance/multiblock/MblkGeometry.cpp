...
#define MBLK_GEOM_MAX_CHARS 128
   ...
   bool is_from_restart)
{
   ...
   bool found = false;
   int i, nb;
   char block_name[MBLK_GEOM_MAX_CHARS];
   ...
   if (d_geom_problem == "CARTESIAN") {
      ...
      for (nb = 0; nb < d_nblocks; ++nb) {
         ...
         // xlo
         snprintf(block_name, MBLK_GEOM_MAX_CHARS, "domain_xlo_%d", nb);
         ...
         // xhi
         snprintf(block_name, MBLK_GEOM_MAX_CHARS, "domain_xhi_%d", nb);
         if (!cart_db->keyExists(block_name)) {
            TBOX_ERROR(d_object_name << ":  "
                                     << "Key data `" << block_name
   ...
   if (d_geom_problem == "WEDGE") {
      ...
      for (nb = 0; nb < d_nblocks; ++nb) {
         ...
         // rmin
         snprintf(block_name, MBLK_GEOM_MAX_CHARS, "rmin_%d", nb);
         ...
         // rmax
         snprintf(block_name, MBLK_GEOM_MAX_CHARS, "rmax_%d", nb);
         if (!wedge_db->keyExists(block_name)) {
            TBOX_ERROR(d_object_name << ":  "
                                     << "Key data `" << block_name
                                     << "' rmax for block " << nb
   ...
   for (nb = 0; nb < d_nblocks; ++nb) {
      ...
      snprintf(block_name, MBLK_GEOM_MAX_CHARS, "rotation_%d", nb);
   ...
   for (nb = 0; nb < d_nblocks; ++nb) {
      ...
      int ln;
      for (ln = 0; ln < 10; ++ln) {
         snprintf(block_name, MBLK_GEOM_MAX_CHARS, "refine_boxes_%d_%d", nb, ln);
         if (db->keyExists(block_name)) {
            ++max_ln;
         }
      }
